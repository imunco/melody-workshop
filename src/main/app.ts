...
import { URL_SCHEME_RXP, MW_DATA_DIR_NAME, MW_DB_FILENAME } from '@common/constants'
...
export const setUserDataPath = () => {
  ...
  global.lxDataPath = path.join(userDataPath, MW_DATA_DIR_NAME)
  // 迁移旧的 LxDatas 目录
  const oldLxDataPath = path.join(userDataPath, 'LxDatas')
  if (!existsSync(global.lxDataPath) && existsSync(oldLxDataPath)) {
    try {
      renameSync(oldLxDataPath, global.lxDataPath)
      console.log('Data directory migrated: LxDatas -> MelodyWorkshopData')
    } catch (err) {
      console.error('Data directory migration failed, using old path:', err)
      global.lxDataPath = oldLxDataPath
    }
  }
...
const backupDB = (backupPath: string) => {
  const dbPath = path.join(global.lxDataPath, MW_DB_FILENAME)
  try {
...
export const initAppSetting = async() => {
  ...
  if (!isInitialized) {
    ...
    if (dbFileExists === false) {
      console.log(`[旋律工坊] 数据库初始化完成，首次启动已创建新数据库: ${path.join(global.lxDataPath, MW_DB_FILENAME)}`)
    }
    if (dbFileExists === null) {
      const backupPath = path.join(global.lxDataPath, `${MW_DB_FILENAME}.${Date.now()}.bak`)
