...
import { APP_BRAND, TRAY_AUTO_ID } from '@common/constants'
...
export const createTray = () => {
  ...
  // tray.setToolTip(APP_BRAND.displayName)
  // createMenu()
...
const defaultTip = APP_BRAND.displayName
const setTip = () => {
  if (!tray) return
...
const init = () => {
  ...
  const shouldShowTray = global.lx.appSetting['tray.enable'] || global.lx.appSetting['tray.isToTray']
  if (isEnableTray !== shouldShowTray) {
    isEnableTray = shouldShowTray
    shouldShowTray ? createTray() : destroyTray()
  }
...
const handleCreateTrayOnClose = () => {
  if (tray && !tray.isDestroyed()) return
  if (!global.lx.appSetting['tray.isToTray'] || isEnableTray) return
  isEnableTray = true
  createTray()
  createMenu()
}
