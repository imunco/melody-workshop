...
dd
  div
    ...
    .gap-top
      base-checkbox(id="setting_to_tray" :model-value="appSetting['tray.enable']" :label="$t('setting__basic_tray_enable')" @update:model-value="updateSetting({'tray.enable': $event})")
    .gap-top
      base-checkbox(id="setting_min_to_tray" :model-value="appSetting['tray.isToTray']" :label="$t('setting__basic_min_to_tray')" @update:model-value="updateSetting({'tray.isToTray': $event})")
    .p.gap-top
...
dd
  h3#melody_workshop_pack {{ $t('setting__melody_pack_title') }}
  div
    .p.small {{ $t('setting__melody_pack_desc') }}
    .p.gap-top
      base-btn.btn(min @click="applyMelodyWorkshopPack") {{ $t('setting__melody_pack_apply') }}
...
import { MELODY_WORKSHOP_THEME_ID } from '@common/constants'
...
export default {
  ...
  setup() {
    ...
    const applyMelodyWorkshopPack = async() => {
      const tid = MELODY_WORKSHOP_THEME_ID
      themeId.value = tid
      applyTheme(tid, appSetting['theme.lightId'], appSetting['theme.darkId'], dataPath)
      updateSetting({
        'theme.id': tid,
        'desktopLyric.style.lyricUnplayColor': 'rgba(255, 255, 255, 0.88)',
        'desktopLyric.style.lyricPlayedColor': 'rgb(61, 154, 150)',
        'desktopLyric.style.lyricPlayedGradientEnd': 'rgb(232, 160, 140)',
        'desktopLyric.style.lyricShadowColor': 'rgba(0, 0, 0, 0.32)',
      })
      void dialog({
        message: t('setting__melody_pack_applied'),
        confirmButtonText: t('alert_button_text'),
      })
    }
