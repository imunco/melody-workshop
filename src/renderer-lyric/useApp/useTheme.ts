...
export default () => {
  ...
  watch(() => [setting['desktopLyric.style.lyricUnplayColor'], setting['desktopLyric.style.lyricPlayedColor'], setting['desktopLyric.style.lyricPlayedGradientEnd'], setting['desktopLyric.style.lyricShadowColor']], ([unplayColor, playedColor, gradientEnd, shadowColor]) => {
    const playedEnd = gradientEnd && String(gradientEnd).trim() ? gradientEnd : playedColor
    window.setLyricColor({
      '--color-lyric-unplay': unplayColor,
      '--color-lyric-played': playedColor,
      '--color-lyric-played-end': playedEnd,
